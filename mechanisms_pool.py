...
FULL_POOL: list[Mechanism] = [
    ...
    Mechanism(
        ...
        explanation="Quand une monnaie se déprécie ou qu'une matière première mondiale flambe, ce renchérissement tend à se répercuter sur les prix intérieurs — l'origine du choc est bien extérieure, mais son ampleur réelle dépend aussi de décisions domestiques (subventions, plafonnement des prix, concurrence) qui peuvent en amortir une partie.",
    ...
    Mechanism(
        ...
        explanation="Le taux auquel un État emprunte se décompose en deux éléments distincts : un niveau général, commun à tous les emprunteurs et déterminé par la politique monétaire et l'inflation anticipée (voir banque_centrale_taux), et une prime de risque spécifique à cet État, qui dépend notamment de la confiance des marchés dans sa capacité à rembourser. Une dégradation de notation ou un doute sur la trajectoire budgétaire fait monter cette seconde composante, indépendamment de ce qui se passe pour les autres emprunteurs.",
        source="Mécanisme des marchés obligataires souverains.",
        mechanism_type=MechanismType.FACT,
        cause_effect="Risque de défaut perçu d'un État ↑ (notation dégradée, doute sur soutenabilité)→prime de risque exigée ↑→coût d'emprunt de cet État ↑ (s'ajoute au niveau général des taux, cause distincte)",
    ...
    Mechanism(
        ...
        explanation="Le coût du transport entre dans le prix final à chaque étape de la chaîne — un blocage portuaire, une hausse du carburant ou un détour de route tend donc à se répercuter jusqu'au consommateur, même si les entreprises intermédiaires peuvent en absorber une partie sur leurs marges, au moins temporairement.",
        source="Mécanisme standard de chaîne logistique.",
        mechanism_type=MechanismType.FACT,
        cause_effect="Coût de transport en hausse→prix final qui tend à augmenter jusqu'au consommateur",
    ...
    Mechanism(
        ...
        base_situation="Une attaque contre un seul pays membre d'une alliance militaire engage la solidarité de tous les autres, sans garantir la même réponse de chacun.",
        explanation="Une clause de défense mutuelle (type article 5 de l'OTAN) engage chaque membre à agir en solidarité si l'un d'eux est attaqué — mais le texte laisse généralement à chaque pays le choix de la nature de sa réponse (militaire, matérielle, diplomatique). L'objectif est de rendre l'attaque d'un membre plus coûteuse et incertaine pour l'agresseur, pas de garantir une riposte militaire automatique et uniforme.",
        source="Mécanisme standard des traités de défense collective.",
        mechanism_type=MechanismType.FACT,
        cause_effect="Attaque contre un membre→obligation politique de solidarité pour tous les autres (nature de la réponse laissée à l'appréciation de chacun)",
